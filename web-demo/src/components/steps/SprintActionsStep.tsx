...
import type { SprintActionsData, SprintActionResult } from '@/types/api';
...
interface SprintActionsStepProps {
  data: SprintActionsData | null;
...
const ACTION_CONFIGS = [
  {
    type: 'add_worklog',
    label: 'Log Work Time',
    description: 'Record time spent on sprint issues',
    icon: Clock,
    color: 'blue',
    toolName: 'addWorklogToJiraIssue',
    isWrite: true,
  },
  {
    type: 'edit_issue',
    label: 'Assign Issues',
    description: 'Update assignee on unassigned sprint tasks',
    icon: UserPlus,
    color: 'blue',
  ...
  },
  {
    ...
    description: 'Create issue links between related tasks',
    icon: Link2,
    color: 'purple',
  ...
  {
    type: 'add_comment',
    label: 'Add Sprint Summary',
    description: 'Post sprint status summary comment on key issues',
    icon: MessageSquare,
    color: 'blue',
...
export function SprintActionsStep({
  data,
  isGenerating,
  requestParams,
  onExecuteActions,
}: SprintActionsStepProps) {
  ...
  if (!data) {
    return (
      <div className="space-y-5 animate-fade-in">
        ...
        <div>
          <h4 className="text-sm font-semibold text-gray-700 mb-1">Sprint Write Operations</h4>
          <p className="text-xs text-gray-500 mb-4">
            Select which sprint actions the agent should execute. Write operations flow through the
...
function ActionResultCard({ action, index }: { action: SprintActionResult; index: number }) {
...
function ActionDetails({
  ...
  const timeSpent = details.timeSpent as string | undefined;
  const issueKey = details.issueKey as string | undefined;
  const assignee = details.assignee as string | undefined;
  const linkType = details.linkType as string | undefined;
  const fromIssue = details.fromIssue as string | undefined;
  const toIssue = details.toIssue as string | undefined;

  return (
    <div className="mt-1.5 flex flex-wrap gap-1">
      {issueKey && <Badge variant="blue">{issueKey}</Badge>}
      {timeSpent && <Badge variant="purple">{timeSpent}</Badge>}
      {assignee && <Badge variant="green">{assignee}</Badge>}
      {linkType && <Badge variant="gray">{linkType}</Badge>}
      {fromIssue && toIssue && (
        <span className="text-xs text-gray-500">
          {fromIssue} &rarr; {toIssue}
        </span>
      )}
    </div>
  );
